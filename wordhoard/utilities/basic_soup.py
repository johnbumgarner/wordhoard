...
def get_single_page_html(url_to_scrape):
    with requests.Session() as session:
        try:
            response = session.get(url_to_scrape, headers=http_headers, allow_redirects=True, verify=True, timeout=30)
            if response.status_code == 403:
                logger.error(f'Response Status Code: {response.status_code}')
                logger.error('HTTP 403 is an HTTP status code meaning access to the requested '
                             'resource is forbidden.')
                logger.error(f'Associated URL: {url_to_scrape}')
            elif response.status_code == 404:
                logger.error(f'Response Status Code: {response.status_code}')
                logger.error('The HTTP 404 Not Found status code means that the file or page that the was requested '
                             'was not found on the server')
                logger.error(f'Associated URL: {url_to_scrape}')
            elif response.status_code == 500:
                logger.error(f'Response Status Code: {response.status_code}')
                logger.error("The HTTP 500 Internal Server Error status code indicates that the server encountered "
                             "an unexpected condition that prevented it from fulfilling the request.")
            elif response.status_code == 503:
                logger.error(f'Response Status Code: {response.status_code}')
                logger.error("The 503 Service Unavailable status code indicates that the server is temporarily unable "
                             "to handle the request")
                logger.error(f'Associated URL: {url_to_scrape}')
            elif response.status_code == 504:
                logger.error(f'Response Status Code: {response.status_code}')
                logger.error("The HTTP 504 Gateway Timeout Error status code indicating that a server, which is "
